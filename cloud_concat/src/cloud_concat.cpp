  ...
  : node_handle_(), private_node_handle_("~"), tf_listener_(), output_frame_("lidar_frame")
{
  ...
  cloud_subscriber1_ = new message_filters::Subscriber<PointCloudMsgT>(node_handle_, "/ns1/velodyne_points", 1);
  cloud_subscriber2_ = new message_filters::Subscriber<PointCloudMsgT>(node_handle_, "/ns2/velodyne_points", 1);
  cloud_synchronizer_ =
...
{
  ros::init(argc, argv, "cloud_concat");
